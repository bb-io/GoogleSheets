...
namespace Apps.GoogleSheets.Auth.OAuth2
{
    public class OAuth2TokenService : BaseInvocable, IOAuth2TokenService, ITokenRefreshable
    {
        ...
        private const string LogPrefix = "[GoogleSheets][OAuth]";
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!values.TryGetValue(RefreshTokenKeyName, out var refreshToken) || string.IsNullOrWhiteSpace(refreshToken))
            {
                throw new PluginMisconfigurationException(
                    "The Google Sheets connection has no refresh token stored, so it cannot be renewed automatically. " +
                    "Please reconnect your Google Sheets connection.");
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                throw new PluginApplicationException(
                    $"Google OAuth token request failed ({(int)response.StatusCode} {response.StatusCode}). {responseContent}");
            }
                ...
                !int.TryParse(expiresInValue, NumberStyles.Integer, CultureInfo.InvariantCulture, out var expiresIn))
            {
                ...
                throw new PluginApplicationException(
                    $"Google OAuth token response did not contain a valid '{ExpiresInKeyName}' value. {responseContent}");
            }

            // Google never returns a refresh token on a refresh; keep the one we already have so the connection stays renewable.
